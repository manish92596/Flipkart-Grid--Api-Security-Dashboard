...
import { Box, Container, Typography, Paper, List, ListItem, ListItemText, Button, Collapse, Grid } from '@mui/material';
...
function Vulnerabilities() {
  ...
  useEffect(() => {
    axios.get('http://localhost:5001/api/vulnerabilities')
      .then(response => {
      ...
      .catch(error => {
        console.error('There was an error fetching the vulnerabilities!', error);
        setVulnerabilities([]);
        setLoading(false);
      });
  }, [vulnerabilities]);
  ...
  const getTotalVulnerabilitiesCount = () => {
    return Object.values(vulnerabilities).reduce((total, routes) => total + routes.length, 0);
  };
                    ...
                    <Collapse in={expanded[key]} timeout="auto" unmountOnExit>
                      <Box 
                        sx={{ 
                          ...
                          overflowY: 'auto', 
                          paddingTop: '1rem',
                          marginBottom: '1rem',  // Add margin here to create space between the routes and the button
                          position: 'absolute',
                          top: 0,
                          left: 0,
                          right: 0,
